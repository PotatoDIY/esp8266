...
void setup() {
  ...
  display.setFont(Dialog_plain_32);//Dialog_plain_8,Dialog_plain_16,Dialog_plain_32,Dialog_plain_48
  display.setTextAlignment(TEXT_ALIGN_LEFT);//TEXT_ALIGN_CENTER，TEXT_ALIGN_RIGHT，TEXT_ALIGN_CENTER_BOTH

  if (APMODE) {
    display.clear();
    display.drawString(0, 0, "ClockAP");
    display.display();
...
void loop() {
  ...
  display.drawString(64, 2, local_time);//字体设置剧中的时候，坐标x应是中心点，即128/2 如果x设置为0，则会显示不全。
